...
// === DAVIDAU AUTOCORRECT + RECONSIDER PATCH ===

// === USER SETTINGS ===
const AUTO_CORRECT_ENABLED = true;
const AUTO_CORRECT_LEVEL = 2; // Medium (balanced randomness)

const RECONSIDER_ENABLED_PARAGRAPH = false;
const RECONSIDER_ENABLED_SENTENCE = false;
const RECONSIDER_SCRAMBLE_LEVEL = 6; // Off (stable tone)

const SYMBOL_FILTER_ENABLED = false;

const SCRAMBLE_SETTINGS = {
  1: { temp: [0.05, 0.95], top_k: [40, 70] },
  2: { temp: [0.5, 1.8], top_k: [30, 90] },
  3: { temp: [0.8, 3.25], top_k: [20, 150] },
  4: { temp: [1.0, 5.0], top_k: [5, 400] },
  5: { useUserMax: true },
  6: { useDefaults: true }
};
...
function detectAnomalies(text) {
  const repeatRegex = /\b(\w+)\s+\1\b/i;
  const badChars = /[\u4E00-\u9FFF]|\uFFFD|�/; // Chinese or replacement chars
  return repeatRegex.test(text) || badChars.test(text);
}

function scrubBadTokens(text) {
  return text.replace(/(\b\w+\s+\1\b)/gi, '') // repetition
             .replace(/[�\uFFFD]/g, '') // replacement chars
             .replace(/[\u4E00-\u9FFF]/g, ''); // CJK if unintended
}

function continueGenerationWithScramble(cleanText, scramble) {
  const scrambleTag = JSON.stringify(scramble);
  return `<<REGEN:${scrambleTag}>>\n${cleanText}`;
}

function shouldAutoCorrect(tokenStream) {
  return detectAnomalies(tokenStream);
}

function applyAutoCorrection(tokenStream) {
  const level = AUTO_CORRECT_LEVEL;
  const scramble = SCRAMBLE_SETTINGS[level];
  const cleaned = scrubBadTokens(tokenStream);
  return continueGenerationWithScramble(cleaned, scramble);
}

function splitByReconsider(text) {
  if (RECONSIDER_ENABLED_SENTENCE) return text.split(/(?<=[.!?])\s+/);
  if (RECONSIDER_ENABLED_PARAGRAPH) return text.split(/\n{2,}/);
  return [text];
}

function resendWithPrior(chunk, scramble) {
  const tag = JSON.stringify(scramble);
  return `<<RECONSIDER:${tag}>>\n${chunk}`;
}

function reconsiderGeneration(text) {
  const pieces = splitByReconsider(text);
  return pieces.map(p => resendWithPrior(p, SCRAMBLE_SETTINGS[RECONSIDER_SCRAMBLE_LEVEL])).join('\n');
}

function onTokenStream(tokenStream) {
  if (AUTO_CORRECT_ENABLED && shouldAutoCorrect(tokenStream)) {
    return applyAutoCorrection(tokenStream);
  }
  return reconsiderGeneration(tokenStream);
}

window.overrideStreamWithDavidAU = onTokenStream;
console.log("✅ DavidAU AutoCorrect Engine: MED + Reconsider per Paragraph loaded");
