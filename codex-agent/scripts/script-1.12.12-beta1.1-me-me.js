...
// if "1" ; "dave systems" are active.

let dave_message_watch="";
let dave_edit_flag=0;
let dave_edit_phrase="";
let dave_holdtext="";
let dave_auto_continue=0;
let dave_edit_start="";

let dave_signal="";
let dave_last_signal=0;
let dave_stop_trigger=0;
let dave_skip_dave_edit=0;

let dave_generate_data={};
...
export function cleanUpMessage(getMessage, isImpersonate, isContinue, displayIncompleteSentences = false, stoppingStrings = null) {
	...
	if (dave_ACTIVE==1) {
	  if (dave_skip_dave_edit==0 && (dave_edit_flag>0 || dave_signal != 0  )) {
		if (dave_edit_flag>0 ) { ({getMessage,continue_mag,messageChunk}=dave_edit(getMessage,"","",3)); }
		else { dave_signal_go(3); }
		}
	dave_message_watch=getMessage;
	}
...
var nIntervId_stream=""; clearInterval(nIntervId_stream);

//
// Main filters / replace ||  LOWERCASE || processed in order listed.
//
...
// Activates System role based on hitting a trigger.
// text added to system role.

var dave_trigger=""; var dave_old_trigger=""; var dave_trigger_count=0;

var dave_replace_phrase=""; var dave_dup_count=0; var dave_old_replace="";

var dave_flag=0; var dave_op=0;

var dave_oldholdtext=""; var dave_holdtext_count=0;

var dave_gib_active=0;

var dave_hard_chop=0; var dave_hard_chop2=0; var dave_soft_chop=0;
...
function dave_reset_hard() {
dave_trigger="";
dave_old_trigger="";
dave_trigger_count=0;
dave_dup_count=0;
dave_old_replace="";
dave_oldholdtext="";
dave_holdtext_count=0;
...
//
// Main "watcher" - Streaming
//

var dave_watch_size=0;
var dave_message_watch_old="";
var dave_match_index=0;
var dave_trigger_point=0;
var dave_trigger_count_old=0;
...
// do not edit.
var dave_error_warn=0;



//  -----------------------------------------------   DO NOT EDIT ABOVE THIS LINE 
//
//  AUTOCORRECT: USER ADJUSTABLE SETTINGS:
//
// Gibbish / Auto-Correct Samplers
// MAIN SYSTEMS/SETTINGS
//
// For "on/off" -> 1=on, 0=off
//

// Turn autocorrect on/off 
// This turns AUTO CORRECT on / off
dave_s['gib_on']=1;

// last XXX characters to "watch" -> characters.
dave_s['sample_size']=500;
...
// next 5 settings in characters.
dave_s['gib_min']=200; 
dave_s['gib_words_repeat_lrg']=5; 
dave_s['gib_words_repeat_sml']=15;
dave_s['gib_words_max_size']=24; 
dave_s['gib_words_max_size_dash']=48;
...
// sentences (scans entire output)
// "min" and "min_size" are in characters.
dave_s['gib_sent_min']=200; 
dave_s['gib_sent_min_size']=20;

// actual sentences required.
dave_s['gib_sent_min_required']=3; 
dave_s['gib_sent_repeat']=2; 
...
// "min" and "min_size" are in characters.
dave_s['gib_para_min_size']=40; 
dave_s['gib_para_min']=200; 

// min # required for checking
dave_s['gib_para_min_required']=2; 
...
// off=0 , on=1
dave_s['auto_continue']=1; 

// set max words, -1 = no max.
// this sets a hard stop separate from EOS, content limits.
dave_s['auto_continue_max']=-1;


//
// regex check -> Allowed characters PER word/entire output
// "Gibbishish" Systems F
//
// NOTE: will block "piñata" , "FAÇADE", "clichés" . => must add the "ñ" and "Ç", "é"
// but will not block "pinata", "FACADE" and "cliches" .
// 
// Contains ONLY allowed characters / symbols in all words, tokens.
//
// If the letter, symbol is NOT here, the word will be auto-rejected.
//
// Only add/take away between the first and last [ ] s unless you know what your are doing.
//

const regex_dave = /^[~`!@#$%^&*()_+–…©™’\'\-\. \n=[\]\{}|;':",.\/<>?a-zA-Z0-9-]+$/;

// turn on/off -> OFF recommended if using other than English (model base and/or language in model) at the moment.
dave_s['gib_regex']=1;


// Turn on/off low quant AND Error changes to TEMP/TOP K
// This activates when there is an error "correction".
// If on, it scrambles temp/top k until NEXT correction OR if passive reconsider is ON -> when it activates.
// temp / topk scramble ON/OFF for ERROR correction.
// 0 = OFF
// 1 = MILD [default] => range .05 to .95 Temp ; k => 40 to 70.
// 2 = MED => range .5 to 1.8 Temp ; k => 30 to 90.
// 3 = HIGH => range .8 to 3.25 Temp ; k => 20 to 150.
// 4 = SPICY => range 1 to 5 Temp ; k => 5 to 400.
// 5 = YOUR defaults MAX
// 6 = Uses defaults DOES NOT SCRAMBLE temp/topk. [added Beta V 1.1]
...
dave_s['gib_error_scramble']=5;

//
// Passive "RECONSIDER" Systems
//  On/Off
// If both off, then systems will only stop/correct when there is an issue.
//

// stop and reassess every paragraph.
dave_s['paragraph_signal_on']=1;

// stop and reassess every sentence.
dave_s['sentence_signal_on']=0;

// temp / topk scramble ON/OFF each "reconsider" automatically.
// 0 = OFF
// 1 = MILD [default] => range .05 to .95 Temp ; k => 40 to 70.
// 2 = MED => range .5 to 1.8 Temp ; k => 30 to 90.
// 3 = HIGH => range .8 to 3.25 Temp ; k => 20 to 150.
...
//
// PRE Gen Sampler Settings
//

// DICE START:

// If set to 1, sets your settings as MAX upper limit (parameters/samplers) of random changes.
// otherwise system will use max settings below.
dave_s['dice_max_on']=0;

// Signal + Signal settings
// Activates on Signal on AND signal condition(s) met.
...
// temp and top key are linked.
dave_s['dice_temp_topk_on']=1; dave_s['dice_temp_topk_chance']=0; dave_s['dice_temp_range']=500; dave_s['dice_topk_range']=150;

dave_s['dice_XTC_on']=1; dave_s['dice_XTC_chance']=5; dave_s['dice_XTC_prob']=100; dave_s['dice_XTC_thre']=100;
...
var dave_active_plus=0;
var dave_active_samplers=0;




//
// Streaming Monitor Function.
//


function watch_dave() {

clearInterval(nIntervId_stream);

if (dave_message_watch.length !=dave_watch_size || dave_message_watch_old !=dave_message_watch ) {

// checksum,lengthsum, case sum
dave_message_watch_old=dave_message_watch; dave_watch_size=dave_message_watch.length; var dl=dave_message_watch.toLowerCase(); var dl_all=dl; dave_flag=0; var t=""; var i=0; var j=0;

// 500 / dave_s['sample_size']  last CHARACTERS / 100 Words => set boundry
if (dave_watch_size>dave_s['sample_size']) {dl=dl.substring(dave_watch_size-dave_s['sample_size']);}

// start at space.
if (dl.indexOf(" ")>-1) {  dl=dl.substring(dl.indexOf(" ")); }


//
// Gibberish Filter ->  go back to last "\n" / "." ->  THEN STOP GEN.
// if it only contains letters => BOOM ||  if the first two = the last two => BOOM
// only does full scan if "error(s)" tripped.
//

var ww=new Array(); var www2=new Array(); dave_gib_active=0;

if (dave_s['gib_on']==1 && dl.length>dave_s['gib_min']) {

var gib_reason=""; var gib=dl; ww=gib.split(" "); dave_error_warn=0;

// WORD CATCH
// catch word, letter, and oddball stuff. ||  no spaces in 500 characters? impossible! || words are over 24 characters in length ? not likely.
// !!! no good for programming code. ||  start from last ... check 50 words.

var k=0; var k1=-1; var sw=new Array(); var duw=[]; var tj=""; j=0;

// Max sample size 50 words. [last 50]
if (ww.length>dave_s['gib_words']) { k=ww.length-dave_s['gib_words']; }

// BOOM -> not enough words!
if (ww.length<10) { k1=1; dave_gib_active=1; t=tj; gib_reason=gib_reason+"Not enough words: " + ww.length + " "; }

// 5 or more repeats of words 5 letters or more? || word over 24 characters / or over 48 characters with "-" s. || Found 15 or more of any word (same) less than 5 characters.
// REGEX => find non english word/characters.  [will only ZAP word]

if (k1<0) {

var regex_count=0; var h=""; var h1=""; var m=0; var k2=0;
for (i=(ww.length-1); i>k; i--) {duw[ww[i]]=0; h1=h1+ww[i]+" "; k2++; if (ww[i].length>m) { m=ww[i].length; } } 

h=h1.replace(new RegExp("-", "gi"), "" ); h=h.replace(new RegExp("—", "gi"), "" );

// power Test / reduce cycles: => shortcut -> no duplicates ? shortcut -> no "bad characters" ( whole string) ?
// any over "max length" => Then do "length" check only.

if ( ((dave_s['gib_regex']==1 && regex_dave.test(h)!=false && regex_dave.test(h1)!=false) || dave_s['gib_regex']==0 ) && Object.keys(duw).length == k2 )  { 

if (m>dave_s['gib_words_max_size'] ) {

for (i=(ww.length-1); i>k; i--) {
// over X size words and repeats
if (ww[i].length>dave_s['gib_words_repeat_lrg'] && ww[i].length>dave_s['gib_words_max_size'] ) {
	if (ww[i].indexOf("-")<0) { k1=1; tj=ww[i]; dave_gib_active=1; t=tj; gib_reason=gib_reason+"LENG: " + ww.length + " OR length word ("+tj+"): " +tj.length ; break; }
	if (ww[i].length>dave_s['gib_words_max_size_dash'] && ww[i].indexOf("-")>0) { k1=1; tj=ww[i]; dave_gib_active=1; t=tj; gib_reason=gib_reason+"LENG: " + ww.length + " OR length word ("+tj+"): " +tj.length ; break; }
}
}
...
// ZAPS only "word" ; standard remove/replace unless overriden BUT more than one -> ZAP it all.
// exception: "-" and "—" and "-WORD"
h=ww[i].replace(new RegExp("-", "gi"), "" ); h=h.replace(new RegExp("—", "gi"), "" );
if (dave_s['gib_regex']==1 && regex_dave.test(ww[i])==false && regex_dave.test(h)==false)  { 
	if (regex_count==0) { dave_flag=2; t=ww[i]; }
	else {k1=1; tj=ww[i]; dave_gib_active=1; t=tj; gib_reason=gib_reason+"Non English Word: " + ww.length + " word ("+tj+"): " +tj.length ; break; } 
regex_count++;
}

// over X size words and repeats
if (ww[i].length>dave_s['gib_words_repeat_lrg']) {
	if (duw[ww[i]]>dave_s['gib_words_repeat_lrg']) {k1=0; tj=ww[i]; dave_gib_active=1; t=tj; gib_reason=gib_reason+" Same SAME SAME words... ("+tj+") "; break;}

	if (ww[i].length>dave_s['gib_words_max_size'] && ww[i].indexOf("\n")<0 && ww[i].indexOf("\r")<0) {
	// if (ww[i].length>dave_s['gib_words_max_size']) {
	if (ww[i].indexOf("-")<0) { k1=1; tj=ww[i]; dave_gib_active=1; t=tj; gib_reason=gib_reason+"LENG: " + ww.length + " OR length word ("+tj+"): " +tj.length ; break; }
	if (ww[i].length>dave_s['gib_words_max_size_dash'] && ww[i].indexOf("-")>0) { k1=1; tj=ww[i]; dave_gib_active=1; t=tj; gib_reason=gib_reason+"LENG: " + ww.length + " OR length word ("+tj+"): " +tj.length ; break; }
	}
}
else {
if (duw[ww[i]]>dave_s['gib_words_repeat_sml']) {k1=1;  tj=ww[i]; dave_gib_active=1; t=tj; gib_reason=gib_reason+"Same same same word, less than 5 characters X+ times ("+duw[ww[i]]+"): " + tj + " " ; break;}
}
}

// end full test else loop
}
...
//
// error systems response:
// ... Can escalate...
//

if (dave_gib_active==1) {dave_error_warn=1;}


// SENTENCE CATCH
// Needs to TEST ALL GEN => If we have sentence duplication, likely catch "paragraph" duplication.

if (dl_all.indexOf(".")>-1 && dl_all.length>dave_s['gib_sent_min'] && dave_gib_active !=1 ) {
var gib2=dl_all; var h=gib2.replace(new RegExp("\n", "gi"), "" ); var www_sen=h.split("."); duw=[];
if (www_sen.length>dave_s['gib_sent_min_required']) {

// preset
for (i=0;i<www_sen.length;i++) { duw[www_sen[i]]=0; }

if (Object.keys(duw).length != www_sen.length) {

for (i=(www_sen.length-1);i>-1;i--) {
if (www_sen[i].length>dave_s['gib_sent_min_size']) { duw[www_sen[i]]++; if (duw[www_sen[i]]>dave_s['gib_sent_repeat']) { dave_gib_active=1; gib_reason=gib_reason+"Sentence Repeat..."; t=www_sen[i]+"."; dave_error_warn=2; break; } } 
}
}
...
// gemma patch.
if (dl_all.indexOf("\r\r")>-1) {sp="\r\r";}

if (dl_all.indexOf(sp)>-1 && dl_all.length>dave_s['gib_para_min'] && dave_gib_active !=1 ) {
var gib2=dl_all; var www_sen=gib2.split(sp); duw=[]; ; var x=0;

if (www_sen.length>dave_s['gib_para_min_required']) {
...
for (i=(www_sen.length-1);i>-1;i--) {
if (www_sen[i].length>dave_s['gib_para_min_size']) { duw[www_sen[i]]++; if (duw[www_sen[i]]>1) { dave_gib_active=1; gib_reason=gib_reason+"Paragraph Repeat..."; t=www_sen[i]+sp; dave_error_warn=2; break; } } 
}
}
...
//
// FIRE GIBBISH CONTROLLER !
//
if (dave_gib_active==1) { console.debug('DAVE: !!!! - Bullsh!t generation detected. KABOOM.  ::' + gib_reason + ' '); dave_flag=90; }
...
if (dave_active_plus==1) {




// end dave_active_plus
// END EXTRA SYSTEMS
}
...
//
// PASSIVE Reconsider Systems
// Becomes Active in sampler area.
// Signals - Causes changes in samplers/parameters/system role
// -> Caught -> Stop -> Change -> Continue || Does NOT 'edit' output but activates stop and restart at "dave_signal_go()".
//

// Paragraph "Reconsider":
if (dave_s['paragraph_signal_on']==1) {
if (dl_all.lastIndexOf("\n\n") >-1 && dl_all.lastIndexOf("\n\n") != dave_last_signal && dave_signal==0 ) { dave_last_signal=0+dl_all.lastIndexOf("\n\n"); dave_signal=1; }
if (dl_all.lastIndexOf("\r\r") >-1 && dl_all.lastIndexOf("\r\r") != dave_last_signal && dave_signal==0 ) { dave_last_signal=0+dl_all.lastIndexOf("\r\r"); dave_signal=1; }
}

// Sentence "Reconsider":
if (dave_s['sentence_signal_on']==1) {
if (dl_all.lastIndexOf(".") >-1 && dl_all.lastIndexOf(".") != dave_last_signal && dave_signal==0 ) { dave_last_signal=0+dl_all.lastIndexOf("."); dave_signal=1; }
}



// GO
// duplicate / system activation/update.

if (dave_flag>0 || dave_signal != 0 ) {

if (dave_flag>0) {

if (t!="") {
dave_edit_phrase=t; dave_edit_flag=dave_flag;

if (dave_old_replace==dave_edit_phrase) {
dave_dup_count++;
}
else {
dave_old_replace=dave_edit_phrase; dave_dup_count=0;
}
...
// Continue
nIntervId_stream = setInterval(watch_dave,20);
}
...
function dave_signal_go(cl) {

// auto continue
dave_auto_continue=1;

// Control signal
dave_stop_trigger=1;

// HALT and restart (auto-continue).
stopGeneration();
...
//
// Main function to "fix" generation.
//
// "cl" -> Call location 1 [ finishGenerating()  ] ,2 [ function onSuccess(data) ] ,3 [  cleanUpMessage function, only "getm" active ] 
//
//
// Cutting...  => dave_edit_flag
//
// PROCESS #1:
// Start at "dave_edit_start" (and remove) ... (OPTIONAL) OR last index of...
// Paragraph =99 / 90 (gibbish) || Sentence =98 || Phrase =97 || Word =96 || 2 words=95 || 3 words=94 || 4 words=93 || 5 words=92 ||  
//
// PROCESS #2:
// REQUIRES: dave_edit_phrase  [now part of process 1]
// dave_soft_chop=1 => remove one extra word () , MODE:  dave_edit_flag ==1 || dave_edit_flag ==2 ||  dave_edit_flag ==4 || dave_edit_flag ==7
//
// PROCESS #3: [works after process 1/2 AND builds on edits from 1/2 ... ]
//
// NO "word/phrase" required => just "chops". [but will not activate without "dave_edit_flag !=0"]
// dave_hard_chop2=1 || dave_edit_flag==91 => Hard chop of paragraph, with fallback to sentence.
// dave_hard_chop2=2 => Cut to sentence only.
//

var lastCL=0;

function dave_edit(getm,contm,chunk,cl) {

// Whoa... needs to make some changes...
stopGeneration();

// RESET: LOCAL
var dave_message=getm; var dave_continue=contm; var dave_chunk=chunk; 
var i=""; var change_replace=""; var soft_mess="";
var sp="\n";

// gemma patch.
if (getm.indexOf("\r\r")>-1) {sp="\r";}

// stop operation from duplicate deleting.
// issue when 3,3, and 1 => ok, but "3" after 1=issues.

if (lastCL==1 && cl==3) {dave_edit_flag=0;cl=-1;}
if (lastCL!=cl) {lastCL=cl;}

// Turned off, faster gen, less chance of "hard stop".  However generation stop/starts are higher when "fixing"
// Critcal End code
// var end_code="<|endoftext|>";

var end_code="";

// replace only. Needs limits.

if (dave_edit_flag ==3) {
dave_message=getm.replace(new RegExp(dave_edit_phrase, "gi"), dave_replace_phrase );  
dave_continue=contm.replace(new RegExp(dave_edit_phrase, "gi"), dave_replace_phrase );  
dave_chunk=chunk.replace(new RegExp(dave_edit_phrase, "gi"), dave_replace_phrase );
}
...
if ( dave_edit_flag !=0 && dave_edit_flag !=3 ) {

// amount of chopping. => 2 runs required for "word"/"phrase" + additional cutting
// if null, then start at 1.

var chp=new Array();

// 1-7 - stop word or stop phrase || reverse course, and STOP.
// REQUIRES "dave_edit_phrase"

// REQUIRED global : dave_replace_phrase
if (dave_edit_flag ==4) {change_replace=dave_replace_phrase;}

// word, phrase, "error" word/phrase 
if (dave_edit_flag ==1 || dave_edit_flag ==2 ||  dave_edit_flag ==4 || dave_edit_flag ==7 || dave_edit_flag==90) {
chp.push(dave_edit_phrase);
}

// end 1-8

// add remove another word, " " boundary.
if (dave_soft_chop==1 || dave_edit_flag==90) {chp.push(" ");}

// paragraph default "99" // "90" [gibbish]
if (dave_edit_flag==99 || dave_edit_flag==90) {chp.push(sp);}

// sentence / could cross para boundry.  => Need boundry check(s).
if (dave_edit_flag==98) {chp.push(".");}

// phrase boundry / might be a bad idea => Need boundry check(s).
if (dave_edit_flag==97) {chp.push(",");}

// one word to 5 word boundary
if (dave_edit_flag==96) {chp.push(" ");}
if (dave_edit_flag==95) {chp.push(" "); chp.push(" "); }
if (dave_edit_flag==94) {chp.push(" "); chp.push(" "); chp.push(" ");}
if (dave_edit_flag==93) {chp.push(" "); chp.push(" "); chp.push(" "); chp.push(" "); }
if (dave_edit_flag==92) {chp.push(" "); chp.push(" "); chp.push(" "); chp.push(" ") ; chp.push(" "); }

// hardchop - should override "array" / nullify.
if (dave_hard_chop2>0 || dave_edit_flag==91) {
if (dave_hard_chop2==2 ) {chp.push(".");}
else {chp.push(sp);}
console.debug('DAVE '+cl+' REPLACE: Hardchop '+dave_hard_chop2+' Done in EDIT AREA ' );
}
...
var getmLC=getm.toLowerCase(); var contmLC=contm.toLowerCase(); var chunkLC=chunk.toLowerCase(); 

if (chp[i] !="" ) {
s1=getmLC.lastIndexOf(chp[i]); s2=contmLC.lastIndexOf(chp[i]); s3=chunkLC.lastIndexOf(chp[i]);

// main
if (s1>-1) {dave_message=getm.substring(0,s1)+change_replace ; dave_message=dave_message.trimEnd();}

// contm
if (s2>-1) {dave_continue=contm.substring(0,s2)+change_replace ; dave_continue=dave_continue.trimEnd();}

// chunk
if (s3>-1) {dave_chunk=chunk.substring(0,s3) +change_replace; dave_chunk=dave_chunk.trimEnd(); }

// end null check.
}

// update
getm=dave_message; contm=dave_continue; chunk=dave_chunk; change_replace="";

// chop loop
}

// END CODE
getm=dave_message.trimEnd()+end_code;  contm=dave_continue.trimEnd()+end_code;  chunk=dave_chunk.trimEnd()+end_code; 
...
// check phrases
if (dave_edit_flag<10 && dave_edit_flag !=0) {console.debug('DAVE: '+cl+' DELETED (PHRASE) a DAVE word function '+dave_edit_flag+': "'+dave_edit_phrase + '"::'+dave_message.substring(dave_message.length-50));}

if (soft_mess!="") {console.debug('DAVE: '+cl+' REPLACED (soft-chop) a DAVE word '+dave_edit_flag+': "'+soft_mess+dave_edit_phrase + '"::'+dave_message.substring(dave_message.length-50));}

if (end_code !="" && dave_message_watch.indexOf(end_code)<0) {dave_message_watch=dave_message_watch.substring(0,dave_message_watch.indexOf(end_code));}
...
// auto continue
// Beta 1.1 Change.
// Add "auto_continue" on/off + MAX check / kill.

var ww=dave_message.split(" ");

if (dave_s['auto_continue']==1 || (dave_s['auto_continue_max']!=-1 && ww.length<dave_s['auto_continue_max']) ) {
dave_auto_continue=1;
}

if (dave_s['auto_continue']==0 || (dave_s['auto_continue_max']!=-1 && ww.length>dave_s['auto_continue_max']) ) {
dave_auto_continue=0;
stopGeneration();
stopGeneration();
}
...
if (cl==1) {dave_edit_phrase=""; dave_edit_flag=0; dave_edit_start=""; dave_hard_chop2=0; dave_soft_chop=0; dave_watch_size=-1; }

// STOP or Continue?
// Controller in "auto continue" TOO.

if (dave_auto_continue==1) {

if ( dave_holdtext.length<5 && dave_holdtext==dave_oldholdtext ) {
dave_holdtext_count++;
if (dave_holdtext_count>4) { dave_auto_continue=0; console.debug('DAVE STOP: Dave reset function'); }
}
else {
if (dave_holdtext!="") { dave_oldholdtext=dave_holdtext; dave_holdtext_count=0; }
}
...
// Who is stopping it? || 0=> edit, 1=> Signal.
dave_stop_trigger=0;
...
// I'll be back...
return {
getMessage:dave_message,
continue_mag:dave_continue,
messageChunk:dave_chunk
}
...
let dave_old_gen_data;
let dave_gen_restart=0;
var gen_data=new Array("temperature","top_k","xtc_probability","xtc_threshold","rep_pen_range","repetition_penalty_range","repeat_last_n","frequency_penalty","presence_penalty","repeat_penalty","rep_pen","repetition_penalty");

function dave_sampler(generate_data) {

	//
	// master patch - All Systems
	//
	...
	if (dave_active_samplers==1) {
	if (dave_gen_restart==0) {
	dave_old_gen_data=[];
	for (var i=0;i<gen_data.length;i++) {	dave_old_gen_data[gen_data[i]]=generate_data[gen_data[i]]; }
	dave_gen_restart=1;
	}
	...
	var risk=(Math.random() * 10); var keep_risk=risk; var CC="";

	var dave_banned=generate_data['banned_strings'];


	// Always Scramble RECONSIDER
	// 0=off, 1=mild, 2=med, 3=high, 4= SPICY

	if (dave_s['reconsider_scramble']!=0 && dave_s['signal_on']==1 && dave_signal!=0) {

	// MILD [default]
	// range .05 to .95 Temp ; k => 40 to 70.
	// Fix Beta 1.1
	if (dave_s['reconsider_scramble']==1) {
	generate_data['temperature']=.05+(Math.floor((Math.random() * 90))/100);
	generate_data['top_k']=40+(Math.floor((Math.random() * 30)) );
	}

	// MED
	// range .5 to 1.8 Temp ; k => 30 to 90.
	if (dave_s['reconsider_scramble']==2) {
	generate_data['temperature']=.5+(Math.floor((Math.random() * 130))/100);
	generate_data['top_k']=30+(Math.floor((Math.random() * 60)) );
	}

	// HIGH
	// range .8 to 3.25 Temp ; k => 20 to 150.
	if (dave_s['reconsider_scramble']==3) {
	generate_data['temperature']=.8+(Math.floor((Math.random() * 250))/100);
	generate_data['top_k']=20+(Math.floor((Math.random() * 130)) );
	}

	// SPICY
	// range 1 to 5 Temp ; k => 5 to 400.
	if (dave_s['reconsider_scramble']==4) {
	generate_data['temperature']=1+(Math.floor((Math.random() * 400))/100);
	generate_data['top_k']=5+(Math.floor((Math.random() * 395)) );
	}

	// YOU ARE IN CONTROL.
	// Your Range
	if (dave_s['reconsider_scramble']==5) {
	generate_data['temperature']=.01+(Math.floor((Math.random() * (generate_data['temperature']*100) ))/100);
	generate_data['top_k']=5+(Math.floor((Math.random() * generate_data['top_k'])) );
	}

	}

	
	// ON error(s) - > scramble TEMP / TOP K

	if (dave_s['gib_error_scramble']!=0 && dave_error_warn !=0) {

	// MILD =1 [default]
	// range .05 to .95 Temp ; k => 40 to 70.
	// Fix Beta 1.1
	if (dave_s['gib_error_scramble']==1) {
	generate_data['temperature']=.05+(Math.floor((Math.random() * 90))/100);
	generate_data['top_k']=40+(Math.floor((Math.random() * 30)) );
	}
	...
	// MED
	// range .5 to 1.8 Temp ; k => 30 to 90.
	if (dave_s['gib_error_scramble']==2) {
	generate_data['temperature']=.5+(Math.floor((Math.random() * 130))/100);
	generate_data['top_k']=30+(Math.floor((Math.random() * 60)) );
	}
	...
	// HIGH
	// range .8 to 3.25 Temp ; k => 20 to 150.
	if (dave_s['gib_error_scramble']==3) {
	generate_data['temperature']=.8+(Math.floor((Math.random() * 250))/100);
	generate_data['top_k']=20+(Math.floor((Math.random() * 130)) );
	}
	...
	// SPICY
	// range 1 to 5 Temp ; k => 5 to 400.
	if (dave_s['gib_error_scramble']==4) {
	generate_data['temperature']=1+(Math.floor((Math.random() * 400))/100);
	generate_data['top_k']=5+(Math.floor((Math.random() * 395)) );
	}
	...
	// YOU ARE IN CONTROL.
	// Your Range
	if (dave_s['gib_error_scramble']==5) {
	generate_data['temperature']=.01+(Math.floor((Math.random() * (generate_data['temperature']*100) ))/100);
	generate_data['top_k']=5+(Math.floor((Math.random() * generate_data['top_k'])) );
	}

	// force greater rep pen range => make model work harder.
	if (dave_error_warn==2) {
	generate_data['rep_pen_range']=1000;
	}
	...
	// SIGNAL(s) - Stop without edit(s)
	// DO NOT do others if "just signal" => dave_stop_trigger = 1
	//

	if (dave_s['signal_on']==1 && dave_signal!=0) {

	CC=CC+(' SIGNALLED STOP Happened' + ":" + dave_signal + ":" +dave_last_signal +" -- ");
	...
	//
	// REPEAT Samplers.
	// keep between changes -only set if signal, but only changes IF signal activates.
	//

	if (dave_generate_data['rep_pen']!=null && 	dave_active_samplers==1 ) {
	...
	//
	// Random Temp/Top K and "RISK"
	// NONE activate if risk is > 10
	//
	// Need lower temp for SYSTEM ROLE TO work better?
	//
	...
	//
	// Trigger -> System Role
	// Can not be used except for "WORD BAN" OR else => need to follow duplicate_remove protocol.
	// This will activate as "second" choice ("remove") to remove with reg edits.
	//
	// NEED to SYNC this better with "other edits" which may/may not be happening.
	// Only "ban" works; because others interfer with other edit/detection systems.
	//

	if (dave_trigger !="" && dave_stop_trigger==0) {

	generate_data['prompt']="Delve into "+dave_trigger+" in vivid and exact detail. " + generate_data['prompt'];
	CC=CC+(' DIRECTIVE: --TRIGGER '+dave_trigger+dave_trigger_count+' --');

	// ban 1,2
	dave_banned.push(dave_trigger);
	CC=CC+(' BANNED Trigger: '+dave_trigger );

	// soft chop ; take one extra word off.
	// !! will transfer to "reg" edit too => EXTRA WORD CHOPPED.
	//if (dave_trigger_count==3 || dave_trigger_count==5) {
	//CC=CC+(' SoftChop Trigger: '+dave_trigger );
	//dave_soft_chop=1;
	//}

	// chop chop... sentence ; will also chop word/phrase first.
	// !! will transfer to "reg" edit too -> MAY CUT TOO MUCH
	//if (dave_trigger_count==5 || dave_trigger_count==6 ) {
	//CC=CC+(' CHOPPED - Sentence (trigger): Word before :'+dave_old_replace );
	//dave_hard_chop2=2;
	//}

	// BIG CHOP - paragraph ; will also chop word/phrase first.
	// !! will transfer to "reg" edit too  -> MAY CUT TOO MUCH
	//if (dave_trigger_count>7) {
	//CC=CC+(' BIG...BIG -- CHOPPED - Paragraph (trigger): Word before :'+dave_old_replace );
	//dave_hard_chop2=1;
	//dave_old_replace="";
	//dave_trigger="";
	//dave_trigger_count=0;
	...
	// end all on/off sampler controls.
	}
	...
	// chop the prompt if
	// TURNED OFF.
	// causes repeat issues sometimes.
	//

	//var oldp=generate_data['prompt'];
	//oldp=oldp.substring(0,oldp.lastIndexOf(" "));
	//generate_data['prompt']=oldp;
	//CC=CC+(' CHOPPED: Word before :'+dave_old_replace );

	// activate sentence CHOP
	//CC=CC+(' CHOPPED - Sentence: Word before :'+dave_old_replace );
	...
	// 7 Step 
	// duplicate count / fixing system
	//

	if (dave_dup_count>0  && dave_stop_trigger==0) {

	generate_data['prompt']="Don't use this word: "+dave_old_replace+", instead go into detail about it. "+ generate_data['prompt'];
	CC=CC+(' DIRECTIVE: --Detail: '+dave_old_replace+' '+dave_dup_count+' --');

	// increase temp and top_K
	// can be switched on/off
	if (dave_active_samplers==1) {
	generate_data['temperature']=1.5+(Math.floor((Math.random() * 300))/100);
	generate_data['top_k']=80+(Math.floor((Math.random() * 150)) );
	}

	// 1,2 -> Try to remove word/phrase.

	// soft chop ; take one extra word off.
	if (dave_dup_count==3 || dave_dup_count==4 ) {
	CC=CC+(' SoftChop Detail: '+dave_dup_count+"::"+dave_old_replace+":: " );
	dave_soft_chop=1;
	}

	// Ban it, 3 or higher.
	if (dave_dup_count>4 ) {
	if (dave_active_samplers==1) { dave_banned.push(dave_old_replace); }

	CC=CC+(' BANNED: '+dave_old_replace );
	}
	
	// Chop Sentence ; will also chop word/phrase first.
	if (dave_dup_count==6 || dave_dup_count==8) {
	CC=CC+(' CHOPPED - Sentence (replace): Word before :'+dave_old_replace );
	dave_hard_chop2=2;
	}

	// BIG CHOP Paragraph  ; will also chop word/phrase first.
	if (dave_dup_count>9) {
	CC=CC+(' BIG...BIG -- CHOPPED - Paragraph: Word before :'+dave_old_replace );
	dave_hard_chop2=1; dave_dup_count=0; dave_old_replace="";
	}
	...
	generate_data['banned_strings']=dave_banned;
	...
	// OTHER SYSTEMS
	//
	// Enforce MAX parameters limits or not.
	// fix gen data issue(s) -> Null AND/OR if "max dice on" => Only max size of random;
	// dave_s['dice_max_on']=1;

	if (dave_active_samplers==1) {

	for (var i=0;i<gen_data.length;i++) {
	if (generate_data[gen_data[i]]==null || (dave_s['dice_max_on']==1 && (generate_data[gen_data[i]]==null || generate_data[gen_data[i]]>dave_old_gen_data[gen_data[i]] )) ) { generate_data[gen_data[i]]=dave_old_gen_data[gen_data[i]]; }
	}
	...
	// Temp / TOP K
	CC=CC+('TEMP/TOPK: '+generate_data['temperature'] + ":" + generate_data['top_k'] );

	// console update
	console.debug('DAVE: '+CC);

	// always reset this:
	dave_stop_trigger=0;


	// end DAVE_ACTIVE on/off
	}


//
// array/function returned... => ALWAYS !
//
